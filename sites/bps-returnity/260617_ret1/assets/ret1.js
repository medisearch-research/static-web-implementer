/* ret1 engine — RETURNITY structure × BP+ content. GSAP3 + ScrollTrigger + Swiper (CDN).
   No top marquee. Per-slide hero entrance · lineup carousel · sticky routine (CSS) · floating inquiry. */
(function () {
  'use strict';
      ...
      new Swiper(el, { loop: true, speed: 1000, effect: 'fade', fadeEffect: { crossFade: true },
        ...
        on: {
          slideChange: function () {
            var c = el.querySelector('[data-slide-no]'); if (c) c.textContent = String(this.realIndex + 1).padStart(2, '0');
            if (this.slides && this.slides[this.activeIndex]) animHeroCap(this.slides[this.activeIndex].querySelector('.hero-cap'));
          }
        } });
      animHeroCap((el.querySelector('.swiper-slide-active') || el).querySelector('.hero-cap'));
    });
  ...
  /* returnity-style hero entrance: image present, text fades up — re-fires every slide */
  function animHeroCap(cap) {
    if (!cap || !hasGSAP) return;
    var t = cap.querySelectorAll('.ey,.en .kin,h1,p,.btn');
    if (!t.length) return;
    if (reduce) { gsap.set(t, { opacity: 1, y: 0 }); return; }
    gsap.fromTo(t, { y: 36, opacity: 0 }, { y: 0, opacity: 1, duration: 0.95, stagger: 0.08, ease: 'power3.out', overwrite: true });
  }

  /* lineup carousel — returnity sec01 (swipeable lineup cards) */
  function initLineup() {
    if (!hasSwiper) return;
    document.querySelectorAll('.linecar').forEach(function (el) {
      var host = el.parentElement;
      new Swiper(el, { slidesPerView: 'auto', spaceBetween: 20, speed: 600, grabCursor: true,
  ...
  /* floating inquiry launcher + scroll-top (returnity #right_quick) */
  function initQuickInquiry() {
    var q = document.querySelector('[data-qinq]');
    if (q) q.addEventListener('click', function (e) {
      var local = document.getElementById('contact');
      if (local) { e.preventDefault(); local.scrollIntoView({ behavior: 'smooth' }); }
    });
    var top = document.querySelector('[data-qtop]');
    if (top) {
  ...
  function initReveals() {
    ...
    /* counters re-fire EVERY time the numbers band scrolls into view (both directions) */
    gsap.utils.toArray('[data-count]').forEach(function (el) {
      var end = parseFloat(el.getAttribute('data-count')) || 0, suf = el.getAttribute('data-suffix') || '', o = { v: 0 };
      var tw = gsap.to(o, { v: end, duration: 1.7, ease: 'power2.out', paused: true, onUpdate: function () { el.textContent = Math.round(o.v).toLocaleString() + suf; } });
      function run() { o.v = 0; el.textContent = '0' + suf; tw.restart(); }
      ScrollTrigger.create({ trigger: el, start: 'top 90%', onEnter: run, onEnterBack: run });
    });
  }
